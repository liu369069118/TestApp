...
#pragma mark -
@implementation XXGP_CommunityCellHeaderView

- (void)setLayout:(XXGP_CommunityLayout *)layout {
    _layout = layout;
...
//    [XXGP_RouterManager openURL:[XXGP_Util getRouterName:@"XXGP_UserInfoController"] withPara:@{@"user_id":NotNullString(_layout.topicModel.userModel.user_id)}];
}

@end

#pragma mark -
@implementation XXGP_CommunityCellTopicView
...
//    [XXGP_RouterManager openURL:[XXGP_Util getRouterName:@"XXGP_TopicDetailViewController"] withPara:@{@"topic_id" : _layout.topicModel.topicTagModel.topic_tag_id}];
}

@end

#pragma mark -
@implementation XXGP_CommunityCellTitleView
...
#pragma mark -
@implementation XXGP_CommunityCellDescribeView
...
- (void)setupLayout {
    XXGP_HyperLinkLabel *describeLabel = [[XXGP_HyperLinkLabel alloc] init];
...
@interface XXGP_CommunityCellPictureView ()

@property (nonatomic, strong) NSMutableArray *pictureItems;

@end

@implementation XXGP_CommunityCellPictureView
...
- (void)setLayout:(XXGP_CommunityLayout *)layout {
    ...
    if (layout.topicModel.img_data_list.count > 0) {
        ...
        for (int i = 0; i < imageCount; i ++) {
            XXGP_CommunityImageModel *imageModel = layout.topicModel.img_data_list[i];
...
#pragma mark -
@implementation XXGP_CommunityCellFunctionView
...
- (void)setupLayout {
    XXGP_CommunityFunctionView *functionView = [[XXGP_CommunityFunctionView alloc] init];
...
#pragma mark -
@implementation XXGP_CommunityCell
...
- (void)setupLayout {
    ...
    XXGP_CommunityCellHeaderView *headerView = [[XXGP_CommunityCellHeaderView alloc] init];
    headerView.layer.masksToBounds = YES;
    [_communityContentView addSubview:headerView];
    _headerView = headerView;
    
    XXGP_CommunityCellTopicView *topicView = [[XXGP_CommunityCellTopicView alloc] init];
    topicView.layer.masksToBounds = YES;
    [_communityContentView addSubview:topicView];
    _topicView = topicView;
    
    XXGP_CommunityCellTitleView *titleView = [[XXGP_CommunityCellTitleView alloc] init];
    titleView.layer.masksToBounds = YES;
    [_communityContentView addSubview:titleView];
    _titleView = titleView;
    
    XXGP_CommunityCellDescribeView *describeView = [[XXGP_CommunityCellDescribeView alloc] init];
    describeView.layer.masksToBounds = YES;
    [_communityContentView addSubview:describeView];
    _describeView = describeView;
    
    XXGP_CommunityCellPictureView *pictureView = [[XXGP_CommunityCellPictureView alloc] init];
    pictureView.layer.masksToBounds = YES;
    [_communityContentView addSubview:pictureView];
    _pictureView = pictureView;
    
    XXGP_CommunityCellFunctionView *functionView = [[XXGP_CommunityCellFunctionView alloc] init];
