...
@interface XXGP_TagHeaderView ()

@property (nonatomic, strong) XXGP_NewsTagBoxView *coinTagBoxView;
@property (nonatomic, strong) XXGP_NewsTagBoxView *topicLeftTagBoxView;
@property (nonatomic, strong) XXGP_NewsTagBoxView *topicCenterTagBoxView;
@property (nonatomic, strong) XXGP_NewsTagBoxView *topicRightTagBoxView;

@end

@implementation XXGP_TagHeaderView
...
- (XXGP_NewsTagBoxView *)coinTagBoxView {
    if (_coinTagBoxView == nil) {
        _coinTagBoxView = [[XXGP_NewsTagBoxView alloc] initWithFrame:CGRectMake(0, 0, 0, 21)];
    }
    return _coinTagBoxView;
}

- (XXGP_NewsTagBoxView *)topicLeftTagBoxView {
    if (_topicLeftTagBoxView == nil) {
        _topicLeftTagBoxView = [[XXGP_NewsTagBoxView alloc] initWithFrame:CGRectMake(0, 0, 0, 21)];
...
- (XXGP_NewsTagBoxView *)topicCenterTagBoxView {
    if (_topicCenterTagBoxView == nil) {
        _topicCenterTagBoxView = [[XXGP_NewsTagBoxView alloc] initWithFrame:CGRectMake(0, 0, 0, 21)];
...
- (XXGP_NewsTagBoxView *)topicRightTagBoxView {
    if (_topicRightTagBoxView == nil) {
        _topicRightTagBoxView = [[XXGP_NewsTagBoxView alloc] initWithFrame:CGRectMake(0, 0, 0, 21)];
...
- (void)setLayout:(XXGP_NewsLayout *)layout {
    ...
    if (layout.news.currencyName.length > 0 || layout.news.topic_tag_list.count > 0) {
        ...
        for (NSInteger index = 0; index < topic_tag_list; index ++) {
            XXGP_NewsTagBoxView *tagBoxView = [self viewWithTag:1000 + index];
...
- (void)setTagOnclickBlock:(void (^)(NSString *url, NSString *tagName, XXGP_NewsModel *newsModel))tagOnclickBlock {
...
@implementation XXGP_CoinRelationView
...
@implementation XXGP_ArticlePictureView
...
@implementation XXGP_UserInfoView
...
@implementation XXGP_DescribeView
...
@interface XXGP_ContentPictureView ()
...
@implementation XXGP_ContentPictureView
...
@implementation XXGP_BottomBarView
...
- (void)setCoinNameStateWithLayout:(XXGP_NewsLayout *)layout {
...
//    hotImageV.backgroundColor = [UIColor redColor];
    ...
    XXGP_TagHeaderView *coinHeaderView = [XXGP_TagHeaderView new];
...
//    _coinRelationView = coinRelationView;
    ...
    // 标题视图
    XXGP_TitleView *titleView = [XXGP_TitleView new];
    titleView.placeholderImage = placeholderImage;
    [_contentView addSubview:titleView];
    _titleView = titleView;
    
    // 文章图片
    XXGP_ArticlePictureView *articlePictureView = [XXGP_ArticlePictureView new];
    ...
    // 用户信息
    XXGP_UserInfoView *userInfoView = [XXGP_UserInfoView new];
    [_contentView addSubview:userInfoView];
    _userInfoView = userInfoView;
    
    // 描述文字
    XXGP_DescribeView *describeView = [XXGP_DescribeView new];
    [_contentView addSubview:describeView];
    _describeView = describeView;
    
    // 内容图片
    XXGP_ContentPictureView *contentPictureView = [XXGP_ContentPictureView new];
    ...
    // 底部信息栏
    XXGP_BottomBarView *bottomBarView = [XXGP_BottomBarView new];
...
- (void)setupLayout {
    ...
    XXGP_NewsView *newsView = [[XXGP_NewsView alloc] init];
