...
#import "SXHGTYProgramProgressHUD.h"
#import <objc/runtime.h>
#define SXHGTYProgramProgressHUDTextMaxWidth ([UIScreen mainScreen].bounds.size.width * 0.75)
...
@property(nonatomic, assign) SXHGTYProgramProgressHUDMode mode;
...
- (instancetype)initWithMode:(SXHGTYProgramProgressHUDMode)mode {
    if (self = [super init]) {
        ...
        if (mode == SXHGTYProgramProgressHUDModeProgressValue) {
...
- (void)didMoveToSuperview{
    ...
    if (self.mode != SXHGTYProgramProgressHUDModeProgressValue) {
        [self startAnimated1];
    }
}
...
- (instancetype)initWithMode:(SXHGTYProgramProgressHUDMode)mode {
    if (self = [super init]) {
        self.mode = mode;
        if (mode == SXHGTYProgramProgressHUDModeLoadingAndText) {
            ...
            _textLabel.preferredMaxLayoutWidth = SXHGTYProgramProgressHUDTextMaxWidth;
            [self addSubview:_textLabel];
            _textLabel.translatesAutoresizingMaskIntoConstraints = NO;
        }
        if (mode == SXHGTYProgramProgressHUDModeLoading) {
            _arcView = [[SSProgressArcView alloc]initWithMode:mode];
            [self addSubview:_arcView];
            _arcView.translatesAutoresizingMaskIntoConstraints = NO;
        }
        if (mode == SXHGTYProgramProgressHUDModeText) {
            _textLabel = [UILabel new];
            _textLabel.numberOfLines = 0;
            _textLabel.textAlignment = NSTextAlignmentCenter;
            _textLabel.font = [UIFont systemFontOfSize:kTextLabelFont];
...
- (void)updateConstraints {
    ...
    if (self.mode == SXHGTYProgramProgressHUDModeText) {
    ...
    if (self.mode == SXHGTYProgramProgressHUDModeLoadingAndText) {
        if (self.text.length) {
    ...
    if (self.mode == SXHGTYProgramProgressHUDModeLoading) {
        NSLayoutConstraint *loadWidth = [NSLayoutConstraint constraintWithItem:self.arcView attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1 constant:kArcRadius*2];
        NSLayoutConstraint *loadHeight = [NSLayoutConstraint constraintWithItem:self.arcView  attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1 constant:kArcRadius*2];
        NSLayoutConstraint *loadTop = [NSLayoutConstraint constraintWithItem:self.arcView   attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeTop multiplier:1 constant:0];
        NSLayoutConstraint *loadLeft = [NSLayoutConstraint constraintWithItem:self.arcView   attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeLeft multiplier:1 constant:0];
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        _arcView = [[SSProgressArcView alloc]initWithMode:SXHGTYProgramProgressHUDModeProgressValue];
        [self addSubview:_arcView];
        _arcView.translatesAutoresizingMaskIntoConstraints = NO;
...
@interface SXHGTYProgramProgressHUD()
...
@implementation SXHGTYProgramProgressHUD
...
- (instancetype)initWithMode:(SXHGTYProgramProgressHUDMode)mode{
    ...
    if (mode == SXHGTYProgramProgressHUDModeLoading) {
        tempView = [[SSProgressLoadTextView alloc]initWithMode:SXHGTYProgramProgressHUDModeLoading];
    }
    
    if (mode == SXHGTYProgramProgressHUDModeLoadingAndText) {
       tempView = [[SSProgressLoadTextView alloc]initWithMode:SXHGTYProgramProgressHUDModeLoadingAndText];
    }
    
    if (mode == SXHGTYProgramProgressHUDModeText) {
        tempView = [[SSProgressLoadTextView alloc]initWithMode:SXHGTYProgramProgressHUDModeText];

    }
    if (mode == SXHGTYProgramProgressHUDModeProgressValue) {
        tempView = [[SSProgressValueView alloc]initWithFrame:CGRectZero];
    }
    if (mode == SXHGTYProgramProgressHUDModeImage) {
...
- (void)showOnView:(UIView *)view animated:(BOOL)animated{
    ...
    [self showOnView:view animated:animated maskType:SXHGTYProgramProgressHUDMaskTypeNone];
...
- (void)showOnView:(UIView *)view animated:(BOOL)animated maskType:(SXHGTYProgramProgressHUDMaskType)maskType {
    ...
    if (maskType != SXHGTYProgramProgressHUDMaskTypeNone) {
        self.maskView = [UIView new];
        if (maskType == SXHGTYProgramProgressHUDMaskTypeBlack) {
...
- (void)setCustomProgress:(CGFloat)customProgress {
    if (self.mode == SXHGTYProgramProgressHUDModeProgressValue&&self.customView) {
...
- (void)setCustomImage:(UIImage *)customImage {
    if (self.mode == SXHGTYProgramProgressHUDModeImage&&self.customView&&customImage) {
        SSProgressImageView *view = (SSProgressImageView *)self.customView;
        view.image = customImage;
    }
}

- (void)setColorType:(SXHGTYProgramProgressHUDColorType)colorType {
    if (colorType == SXHGTYProgramProgressHUDColorTypeBlack) {
...
@implementation NSObject (SXHGTYProgramProgressHUD)
...
- (void)showLoadingWithTag:(NSInteger)tag {
    [self showLoadingWithTag:tag maskType:SXHGTYProgramProgressHUDMaskTypeNone];
}
- (void)showLoadingWithTag:(NSInteger)tag maskType:(SXHGTYProgramProgressHUDMaskType)maskType {
    [self showHUDWithMode:SXHGTYProgramProgressHUDModeLoading text:nil tag:tag maskType:maskType];
}
...
- (void)showLoadingText:( NSString *)text tag:(NSInteger)tag {
    ...
    [self showLoadingText:text tag:tag maskType:SXHGTYProgramProgressHUDMaskTypeNone];
}
- (void)showLoadingText:(NSString *)text tag:(NSInteger)tag maskType:(SXHGTYProgramProgressHUDMaskType)maskType {
     [self showHUDWithMode:SXHGTYProgramProgressHUDModeLoadingAndText text:text tag:tag maskType:maskType];
}
...
- (void)showText:(NSString *)text finished:(void (^ __nullable)(void))finished {
    [self showText:text maskType:SXHGTYProgramProgressHUDMaskTypeNone finished:finished];
}

- (void)showText:(NSString *)text maskType:(SXHGTYProgramProgressHUDMaskType)maskType finished:(void(^ __nullable)(void))finished {
    SXHGTYProgramProgressHUD *hud = [self showHUDWithMode:SXHGTYProgramProgressHUDModeText text:text tag:0 maskType:maskType];
...
- (void)showProgress:(CGFloat)progress finished:(void (^ __nullable)(void))finished {
    [self showProgress:progress text:nil maskType:SXHGTYProgramProgressHUDMaskTypeNone finished:finished];
}
...
- (void)showProgress:(CGFloat)progress text:(NSString *)text finished:(void (^ __nullable)(void))finished {
    [self showProgress:progress text:text maskType:SXHGTYProgramProgressHUDMaskTypeNone finished:finished];
}

- (void)showProgress:(CGFloat)progress text:(NSString * _Nullable)text maskType:(SXHGTYProgramProgressHUDMaskType)maskType finished:(void (^ _Nullable)(void))finished {
    SXHGTYProgramProgressHUD *hud = [self progressValueHUDWithMaskType:maskType];
    hud.customText = text;
    __weak SXHGTYProgramProgressHUD *weakHud = hud;
...
- (void)showHUDWithImageName:(NSString *)imageName text:(NSString *)text finished:( void(^ __nullable)(void))finished {
    ...
    [self showHUDWithImageName:imageName text:text maskType:SXHGTYProgramProgressHUDMaskTypeNone finished:finished];
}
- (void)showHUDWithImageName:(NSString *)imageName text:(NSString *)text maskType:(SXHGTYProgramProgressHUDMaskType)maskType finished:(void (^ _Nullable)(void))finished {
    SXHGTYProgramProgressHUD *hud = [self showHUDWithMode:SXHGTYProgramProgressHUDModeImage text:text tag:0 maskType:maskType];
...
- (void)hideHUDAnimated:(BOOL)animated tag:(NSInteger)tag finished:(void (^ __nullable)(void))finished {
    SXHGTYProgramProgressHUD *hud = [[self hudDict]objectForKey:[NSString stringWithFormat:@"%ld",tag]];
    [hud hideViewAnimated:animated completion:finished];
}

#pragma mark - method

- (SXHGTYProgramProgressHUD *)showHUDWithMode:(SXHGTYProgramProgressHUDMode)mode text:(NSString *)text tag:(NSInteger)tag maskType:(SXHGTYProgramProgressHUDMaskType)maskType{
    if (![self getSuperView]) return nil;
    SXHGTYProgramProgressHUD *hud = [[SXHGTYProgramProgressHUD alloc]initWithMode:mode];
//    hud.colorType = SXHGTYProgramProgressHUDColorTypeBlack;
    hud.customText = text;
    ...
    if (tag>0||tag==-1) {
        if ([[[self hudDict]allKeys]containsObject:tagKey]) {
            SXHGTYProgramProgressHUD *t = [[self hudDict]objectForKey:tagKey];
...
- (SXHGTYProgramProgressHUD *)progressValueHUDWithMaskType:(SXHGTYProgramProgressHUDMaskType)maskType {
    if (!objc_getAssociatedObject(self, _cmd)) {
        SXHGTYProgramProgressHUD *hud =  [[SXHGTYProgramProgressHUD alloc]initWithMode:SXHGTYProgramProgressHUDModeProgressValue];
        [hud showOnView:[self getSuperView] animated:YES maskType:maskType];
