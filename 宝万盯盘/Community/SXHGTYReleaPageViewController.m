//
//  SXHGTYReleaPageViewController.m
//  SCDBYIProject
//
//  Created by jo Hebing on 2020/1/6.
...
@interface SXHGTYReleaPageViewController ()<UITextViewDelegate,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
@implementation SXHGTYReleaPageViewController
...
- (void)sendFeedBack{
  BOOL isFirst =   [[NSUserDefaults standardUserDefaults] objectForKey:@"isLoad"];
    if (!isFirst) {
        SXHGTYAlertViewController *alertVC = [SXHGTYAlertViewController alertControllerWithTitle:@"发表规则" message:@"1.用户对其发表的信息负责，应尽可能提供详尽、真实、准确的材料,不得发表不真实的、有歧义的信息，绝对禁止发表误导性的、恶意的消息产品。  \n   2.用户不得在平台内发表与本应用无关的内容，我们会在8小时内对用户的内容进行审核,审核通过后才可发表到产品平台内.   \n     3.用户如发现其他用户发表的内容不当，应在内容页进行举报!收到举报后我们会在8小时内对内容进行重新审核!" preferredStyle:UIAlertControllerStyleAlert];
        alertVC.source = SXHGTYAlertViewSourceTypeReleas;
    ...
    if (self.SXHG_ConyTextV.text.length == 0) {
        ...
        [SXHGTYProgramProgressHUD showLoadingText:@"发布中..."];
        
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            
            [SXHGTYProgramProgressHUD hideHUDAnimated:YES];
