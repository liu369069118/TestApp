...
- (void)reloadPortraitFrame:(NSInteger)page{
    ...
    [UIView animateWithDuration:PhotoBrowerTransformTime animations:^{
        ...
        _numView.frame = CGRectMake(16, kHCStatusBarHeight, 45, 25);
    }];
    ...
    [UIView animateWithDuration:PhotoBrowerTransformTime animations:^{
        ...
        _operationBtn.frame = (CGRect){{ScreenWidth - 35 - 15,iPhoneX_Series?45:25},{35,20}};
        _operationBtn.center = (CGPoint){ScreenWidth - 35 - 15 + 35 / 2,iPhoneX_Series?(45 + 10):(15 + 10)};
    }];
}
...
- (void)reloadLandscapeIsLeftFrame:(NSInteger)page{
    ...
    [UIView animateWithDuration:PhotoBrowerBrowerTime animations:^{
        ...
        _operationBtn.center = CGPointMake(ScreenWidth - (10 + (iPhoneX_Series ? 45:25)), ScreenHeight - 35 * 0.5 - 15);
    }];
}
...
- (void)reloadLandscapeIsRightFrame:(NSInteger)page{
    ...
    [UIView animateWithDuration:PhotoBrowerBrowerTime animations:^{
        _operationBtn.transform = CGAffineTransformMakeRotation(- M_PI * 0.5);
        _operationBtn.frame = CGRectMake(0, 0, 20, 35);
        _operationBtn.center = CGPointMake(10 + (iPhoneX_Series ? 45 : 25),  35 * 0.5 + 15);
    }];
}
...
- (void)initializePageView{
    ...
    numView.frame = CGRectMake(16, kHCStatusBarHeight, 60, 25);
...
- (void)initializeOperationView{
    ...
    [operationBtn setFrame:(CGRect){{ScreenWidth - 35 - 15,iPhoneX_Series?45:25},{35,20}}];
...
- (void)operationBtnIBAction {
    ...
    if (_isUseCustomActionSheet) {
        if(!_isNeedPictureLongPress) return;
        
        HBActionSheet *actionSheet = [[HBActionSheet alloc] initActionSheetWithTitle:nil descriptiveText:nil cancelButtonTitle:@"取消" destructiveButtonTitles:nil otherButtonTitles:@[@"保存图片到本地"] itemAction:^(HBActionSheet *actionSheet, NSString *title, NSInteger index) {
            if ([title isEqualToString:@"保存图片到本地"]) {
                [weakSelf clickSavaPhotoToLocal:actionSheet];
            }
        }];
        ...
        if(_actionSheetArr.count != 0){ // 如果是自定义的 选项
            KNActionSheet *actionSheet = [[KNActionSheet alloc] initWithCancelTitle:nil otherTitleArr:[weakSelf.actionSheetArr copy] actionBlock:^(KNActionSheet *currentActionSheet, NSInteger buttonIndex) {
                // 让代理知道 是哪个按钮被点击了
            ...
            KNActionSheet *actionSheet = [[KNActionSheet alloc] initWithCancelTitle:nil destructiveTitle:@"删除" otherTitleArr:@[@"保存图片",@"转发微博",@"赞"]  actionBlock:^(KNActionSheet *currentActionSheet, NSInteger buttonIndex) {
                // 让代理知道 是哪个按钮被点击了
                if([weakSelf.delegate respondsToSelector:@selector(photoBrowerRightOperationActionWithIndex:)]){
                    [weakSelf.delegate photoBrowerRightOperationActionWithIndex:buttonIndex];
                }
...
#pragma mark - 删除图片
                        ...
                        break;
                    case 1:{ // 下载图片
                        [weakSelf clickSavaPhotoToLocal:currentActionSheet];
                    }
                        /**
                         *  剩下的需要自己去实现
                         */
...
- (void)clickSavaPhotoToLocal:(id)actionSheet {
    if(![self isAuthPhoto]){
        if ([actionSheet isKindOfClass:[KNActionSheet class]]) {
            [(KNActionSheet *)actionSheet dismiss];
        } else {
            [(HBActionSheet *)actionSheet dismissAction];
        }
        return ;
    }
    ...
    if(items.url){ // 如果是网络图片
        [SDImageCache.sharedImageCache diskImageExistsWithKey:items.url completion:^(BOOL isInCache) {
            if(!isInCache){
                [[KNToast shareToast] initWithText:PhotoSaveImageFailureReason];
                return ;
            }else{
                @WeakObj(self);
                [SDImageCache.sharedImageCache queryCacheOperationForKey:items.url done:^(UIImage * _Nullable image, NSData * _Nullable data, SDImageCacheType cacheType) {
                    if (image.images != nil) {
                        [selfWeak savePhotoToLocation:data]; // 将 gif 图片存入本地
                    } else {
                        UIImage *image = [SDImageCache.sharedImageCache imageFromDiskCacheForKey:items.url];
                        dispatch_async(dispatch_get_main_queue(), ^{
                            UIImageWriteToSavedPhotosAlbum(image, selfWeak, @selector(image:didFinishSavingWithError:contextInfo:), NULL);
                        });
                    }
                }];
            }
...
- (BOOL)isAuthPhoto {
    ...
    if (status == PHAuthorizationStatusAuthorized) { // 用户允许访问相册
        photoStatus = YES;
        ...
        [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
            if (status == PHAuthorizationStatusAuthorized) {
                dispatch_main_async_safe(^{
                    [selfWeak operationBtnIBAction];
                });
            }
        }];
    } else {
        HBActionSheet *actionSheet = [[HBActionSheet alloc] initActionSheetWithTitle:nil descriptiveText:@"没有权限无法保存图片，是否授权相机权限给HOLD？" cancelButtonTitle:@"取消" destructiveButtonTitles:nil otherButtonTitles:@[@"授权"] itemAction:^(HBActionSheet *actionSheet, NSString *title, NSInteger index) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if(_isFirstShow == false){
        ...
        if(PhotoOrientationUnknown ||
           PhotoOrientationLandscapeIsPortrait ||
           PhotoOrientationLandscapeIsPortraitUpsideDown){
            _currentIndex = scrollView.contentOffset.x / (ScreenWidth + PhotoBrowerMargin);
        }else{
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    ...
    if (scrollView.contentOffset.x < -20 ||
        scrollView.contentOffset.x > (_itemsArr.count - 1) * (ScreenWidth + PhotoBrowerMargin) + 20) {
        [self dismiss];
        return;
    }
}
    ...
    [SDImageCache.sharedImageCache diskImageExistsWithKey:items.url completion:^(BOOL isInCache) {
        if(isInCache){
            if([[[[items.url lastPathComponent] pathExtension] lowercaseString] isEqualToString:@"gif"]){ // gif 图片
                NSData *data = UIImageJPEGRepresentation([SDImageCache.sharedImageCache imageFromCacheForKey:items.url], 1.f);
                
                tempView.image = [self imageFromGifFirstImage:data]; // 获取图片的第一帧
            }else{ // 普通图片
                tempView.image = [SDImageCache.sharedImageCache imageFromCacheForKey:items.url];
            }
        }else{
