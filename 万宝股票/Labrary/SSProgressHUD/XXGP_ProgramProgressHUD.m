...
#define XXGP_ProgramProgressHUDTextMaxWidth ([UIScreen mainScreen].bounds.size.width * 0.75)
...
@property(nonatomic, assign) XXGP_ProgramProgressHUDMode mode;
...
- (instancetype)initWithMode:(XXGP_ProgramProgressHUDMode)mode {
    if (self = [super init]) {
        ...
        if (mode == XXGP_ProgramProgressHUDModeProgressValue) {
...
- (void)didMoveToSuperview{
    ...
    if (self.mode != XXGP_ProgramProgressHUDModeProgressValue) {
        [self startAnimated1];
    }
}
...
- (instancetype)initWithMode:(XXGP_ProgramProgressHUDMode)mode {
    if (self = [super init]) {
        self.mode = mode;
        if (mode == XXGP_ProgramProgressHUDModeLoadingAndText) {
            ...
            _textLabel.preferredMaxLayoutWidth = XXGP_ProgramProgressHUDTextMaxWidth;
            [self addSubview:_textLabel];
            _textLabel.translatesAutoresizingMaskIntoConstraints = NO;
        }
        if (mode == XXGP_ProgramProgressHUDModeLoading) {
            _arcView = [[SSProgressArcView alloc]initWithMode:mode];
            [self addSubview:_arcView];
            _arcView.translatesAutoresizingMaskIntoConstraints = NO;
        }
        if (mode == XXGP_ProgramProgressHUDModeText) {
            _textLabel = [UILabel new];
            _textLabel.numberOfLines = 0;
            _textLabel.textAlignment = NSTextAlignmentCenter;
            _textLabel.font = [UIFont systemFontOfSize:kTextLabelFont];
...
- (void)updateConstraints {
    ...
    if (self.mode == XXGP_ProgramProgressHUDModeText) {
    ...
    if (self.mode == XXGP_ProgramProgressHUDModeLoadingAndText) {
        if (self.text.length) {
    ...
    if (self.mode == XXGP_ProgramProgressHUDModeLoading) {
        NSLayoutConstraint *loadWidth = [NSLayoutConstraint constraintWithItem:self.arcView attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1 constant:kArcRadius*2];
        NSLayoutConstraint *loadHeight = [NSLayoutConstraint constraintWithItem:self.arcView  attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1 constant:kArcRadius*2];
        NSLayoutConstraint *loadTop = [NSLayoutConstraint constraintWithItem:self.arcView   attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeTop multiplier:1 constant:0];
        NSLayoutConstraint *loadLeft = [NSLayoutConstraint constraintWithItem:self.arcView   attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeLeft multiplier:1 constant:0];
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        _arcView = [[SSProgressArcView alloc]initWithMode:XXGP_ProgramProgressHUDModeProgressValue];
        [self addSubview:_arcView];
        _arcView.translatesAutoresizingMaskIntoConstraints = NO;
...
@interface XXGP_ProgramProgressHUD()
...
@implementation XXGP_ProgramProgressHUD
...
- (instancetype)initWithMode:(XXGP_ProgramProgressHUDMode)mode{
    ...
    if (mode == XXGP_ProgramProgressHUDModeLoading) {
        tempView = [[SSProgressLoadTextView alloc]initWithMode:XXGP_ProgramProgressHUDModeLoading];
    }
    
    if (mode == XXGP_ProgramProgressHUDModeLoadingAndText) {
       tempView = [[SSProgressLoadTextView alloc]initWithMode:XXGP_ProgramProgressHUDModeLoadingAndText];
    }
    
    if (mode == XXGP_ProgramProgressHUDModeText) {
        tempView = [[SSProgressLoadTextView alloc]initWithMode:XXGP_ProgramProgressHUDModeText];

    }
    if (mode == XXGP_ProgramProgressHUDModeProgressValue) {
        tempView = [[SSProgressValueView alloc]initWithFrame:CGRectZero];
    }
    if (mode == XXGP_ProgramProgressHUDModeImage) {
...
- (void)showOnView:(UIView *)view animated:(BOOL)animated{
    ...
    [self showOnView:view animated:animated maskType:XXGP_ProgramProgressHUDMaskTypeNone];
...
- (void)showOnView:(UIView *)view animated:(BOOL)animated maskType:(XXGP_ProgramProgressHUDMaskType)maskType {
    ...
    if (maskType != XXGP_ProgramProgressHUDMaskTypeNone) {
        self.maskView = [UIView new];
        if (maskType == XXGP_ProgramProgressHUDMaskTypeBlack) {
...
- (void)setCustomProgress:(CGFloat)customProgress {
    if (self.mode == XXGP_ProgramProgressHUDModeProgressValue&&self.customView) {
...
- (void)setCustomImage:(UIImage *)customImage {
    if (self.mode == XXGP_ProgramProgressHUDModeImage&&self.customView&&customImage) {
        SSProgressImageView *view = (SSProgressImageView *)self.customView;
        view.image = customImage;
    }
}

- (void)setColorType:(XXGP_ProgramProgressHUDColorType)colorType {
    if (colorType == XXGP_ProgramProgressHUDColorTypeBlack) {
...
@implementation NSObject (XXGP_ProgramProgressHUD)
...
- (void)showLoadingWithTag:(NSInteger)tag {
    [self showLoadingWithTag:tag maskType:XXGP_ProgramProgressHUDMaskTypeNone];
}
- (void)showLoadingWithTag:(NSInteger)tag maskType:(XXGP_ProgramProgressHUDMaskType)maskType {
    [self showHUDWithMode:XXGP_ProgramProgressHUDModeLoading text:nil tag:tag maskType:maskType];
}
...
- (void)showLoadingText:( NSString *)text tag:(NSInteger)tag {
    ...
    [self showLoadingText:text tag:tag maskType:XXGP_ProgramProgressHUDMaskTypeNone];
}
- (void)showLoadingText:(NSString *)text tag:(NSInteger)tag maskType:(XXGP_ProgramProgressHUDMaskType)maskType {
     [self showHUDWithMode:XXGP_ProgramProgressHUDModeLoadingAndText text:text tag:tag maskType:maskType];
}
...
- (void)showText:(NSString *)text finished:(void (^ __nullable)(void))finished {
    [self showText:text maskType:XXGP_ProgramProgressHUDMaskTypeNone finished:finished];
}

- (void)showText:(NSString *)text maskType:(XXGP_ProgramProgressHUDMaskType)maskType finished:(void(^ __nullable)(void))finished {
    XXGP_ProgramProgressHUD *hud = [self showHUDWithMode:XXGP_ProgramProgressHUDModeText text:text tag:0 maskType:maskType];
...
- (void)showProgress:(CGFloat)progress finished:(void (^ __nullable)(void))finished {
    [self showProgress:progress text:nil maskType:XXGP_ProgramProgressHUDMaskTypeNone finished:finished];
}
...
- (void)showProgress:(CGFloat)progress text:(NSString *)text finished:(void (^ __nullable)(void))finished {
    [self showProgress:progress text:text maskType:XXGP_ProgramProgressHUDMaskTypeNone finished:finished];
}

- (void)showProgress:(CGFloat)progress text:(NSString * _Nullable)text maskType:(XXGP_ProgramProgressHUDMaskType)maskType finished:(void (^ _Nullable)(void))finished {
    XXGP_ProgramProgressHUD *hud = [self progressValueHUDWithMaskType:maskType];
    hud.customText = text;
    __weak XXGP_ProgramProgressHUD *weakHud = hud;
...
- (void)showHUDWithImageName:(NSString *)imageName text:(NSString *)text finished:( void(^ __nullable)(void))finished {
    ...
    [self showHUDWithImageName:imageName text:text maskType:XXGP_ProgramProgressHUDMaskTypeNone finished:finished];
}
- (void)showHUDWithImageName:(NSString *)imageName text:(NSString *)text maskType:(XXGP_ProgramProgressHUDMaskType)maskType finished:(void (^ _Nullable)(void))finished {
    XXGP_ProgramProgressHUD *hud = [self showHUDWithMode:XXGP_ProgramProgressHUDModeImage text:text tag:0 maskType:maskType];
...
- (void)hideHUDAnimated:(BOOL)animated tag:(NSInteger)tag finished:(void (^ __nullable)(void))finished {
    XXGP_ProgramProgressHUD *hud = [[self hudDict]objectForKey:[NSString stringWithFormat:@"%ld",tag]];
    [hud hideViewAnimated:animated completion:finished];
}

#pragma mark - method

- (XXGP_ProgramProgressHUD *)showHUDWithMode:(XXGP_ProgramProgressHUDMode)mode text:(NSString *)text tag:(NSInteger)tag maskType:(XXGP_ProgramProgressHUDMaskType)maskType{
    if (![self getSuperView]) return nil;
    XXGP_ProgramProgressHUD *hud = [[XXGP_ProgramProgressHUD alloc]initWithMode:mode];
//    hud.colorType = XXGP_ProgramProgressHUDColorTypeBlack;
    hud.customText = text;
    ...
    if (tag>0||tag==-1) {
        if ([[[self hudDict]allKeys]containsObject:tagKey]) {
            XXGP_ProgramProgressHUD *t = [[self hudDict]objectForKey:tagKey];
...
- (XXGP_ProgramProgressHUD *)progressValueHUDWithMaskType:(XXGP_ProgramProgressHUDMaskType)maskType {
    if (!objc_getAssociatedObject(self, _cmd)) {
        XXGP_ProgramProgressHUD *hud =  [[XXGP_ProgramProgressHUD alloc]initWithMode:XXGP_ProgramProgressHUDModeProgressValue];
        [hud showOnView:[self getSuperView] animated:YES maskType:maskType];
