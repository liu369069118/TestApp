...
#import "SXHGTYEditOptionalViewController.h"
#import "SXHGTYOPtiondicorationEditCell.h"

static const CGFloat topViewHeight = 30;
static const CGFloat bottomViewHeight = 40;

@interface SXHGTYEditOptionalViewController () <UITableViewDelegate, UITableViewDataSource, UIGestureRecognizerDelegate>
...
@implementation SXHGTYEditOptionalViewController
...
- (void)getRequestMessageListDatass {
    NSMutableArray *all = [WBUitl mainDataList];
    ...
    if (self.SXHG_ContetListGroups.count == 0) {
        [SXHGTYProgramProgressHUD showText:@"暂未添加股票"];
    }
...
- (void)initSCommonViews {
    ...
    self.contentListTable = ({
        ...
        [tableView registerNib:[UINib nibWithNibName:@"SXHGTYOPtiondicorationEditCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"SXHGTYOPtiondicorationEditCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    SXHGTYOPtiondicorationEditCell *cell = [tableView dequeueReusableCellWithIdentifier:@"SXHGTYOPtiondicorationEditCell"];
    if (self.SXHG_ContetListGroups.count > 0) {
        cell.model = self.SXHG_ContetListGroups[indexPath.row];
    }
    //置顶事件的回调
    WEAKSELF;
    cell.buttonClickBlock = ^(SXHGTYOPtiondicorationEditCell *cell) {
        //根据cell获取下标
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    SXHGTYOPtiondicorationEditCell *cell = (SXHGTYOPtiondicorationEditCell *) [tableView cellForRowAtIndexPath:indexPath];
    [cell layoutSubviews];
    //1.判断此时是删除队列是否为空
