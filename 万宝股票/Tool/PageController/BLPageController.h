...
@class BLPageController;

FOUNDATION_EXTERN NSString *const _Nonnull BLControllerDidAddToSuperViewNotification;
FOUNDATION_EXTERN NSString *const _Nonnull BLControllerDidFullyDisplayedNotification;

/*
 *  BLPageController 的缓存设置，默认缓存为无限制，当收到 memoryWarning 时，会自动切换到低缓存模式 (BLPageControllerCachePolicyLowMemory)，并在一段时间后切换到 High .
 收到多次警告后，会停留在到 BLPageControllerCachePolicyLowMemory 不再增长
 *
 ...
 */
typedef NS_ENUM(NSUInteger, BLPageControllerCachePolicy) {
    BLPageControllerCachePolicyDisabled  = -1,   // Disable Cache
    BLPageControllerCachePolicyNoLimit   = 0,    // No limit
    BLPageControllerCachePolicyLowMemory = 1,    // Low Memory but may block when scroll
    BLPageControllerCachePolicyBalanced  = 3,    // Balanced ↑ and ↓
    BLPageControllerCachePolicyHigh      = 5     // High
};

typedef NS_ENUM(NSUInteger, BLPageControllerPreloadPolicy) {
    BLPageControllerPreloadPolicyNever     = 0, // Never pre-load controller.
    BLPageControllerPreloadPolicyNeighbour = 1, // Pre-load the controller next to the current.
    BLPageControllerPreloadPolicyNear      = 2  // Pre-load 2 controllers near the current.
};
...
/**
 *  To inform how many child controllers will in `BLPageController`.
 *
 *  @param pageController The parent controller.
 *
 *  @return The value of child controllers's count.
 */
- (NSInteger)numbersOfChildControllersInPageController:(BLPageController * _Nonnull)pageController;
 ...
 */
- (__kindof UIViewController * _Nonnull)pageController:(BLPageController * _Nonnull)pageController viewControllerAtIndex:(NSInteger)index;

/**
 *  Each title you wanna show in the `BLMenuView`
 *
 *  @param pageController The parent controller.
 *  @param index          The index of title.
 *
 *  @return A `NSString` value to show at the top of `BLPageController`.
 */
- (NSString * _Nonnull)pageController:(BLPageController * _Nonnull)pageController titleAtIndex:(NSInteger)index;
 ...
 *
 *  @param pageController The parent controller (BLPageController)
 ...
 */
- (void)pageController:(BLPageController * _Nonnull)pageController lazyLoadViewController:(__kindof UIViewController * _Nonnull)viewController withInfo:(NSDictionary * _Nonnull)info;
 ...
 *  @param info           A dictionary that includes some infos, such as: `index` / `title`
 */
- (void)pageController:(BLPageController * _Nonnull)pageController willCachedViewController:(__kindof UIViewController * _Nonnull)viewController withInfo:(NSDictionary * _Nonnull)info;
 ...
 *  @param info           A dictionary that includes some infos, such as: `index` / `title`
 */
- (void)pageController:(BLPageController * _Nonnull)pageController willEnterViewController:(__kindof UIViewController * _Nonnull)viewController withInfo:(NSDictionary * _Nonnull)info;
 ...
 *  @param info           A dictionary that includes some infos, such as: `index` / `title`
 */
- (void)pageController:(BLPageController * _Nonnull)pageController didEnterViewController:(__kindof UIViewController * _Nonnull)viewController withInfo:(NSDictionary * _Nonnull)info;

@end

@interface BLPageController : UIViewController <BLMenuViewDelegate, BLMenuViewDataSource, UIScrollViewDelegate, BLPageControllerDataSource, BLPageControllerDelegate>

@property (nonatomic, weak) id<BLPageControllerDelegate> _Nullable delegate;
@property (nonatomic, weak) id<BLPageControllerDataSource> _Nullable dataSource;
...
@property (nonatomic, strong, readonly) UIViewController * _Nullable currentViewController;
 ...
 */
@property (nonatomic, assign) BLMenuViewStyle menuViewStyle;

@property (nonatomic, assign) BLMenuViewLayoutMode menuViewLayoutMode;
 ...
 *  See `BLPageConst.h` for more information.
 */
...
@property (nonatomic, assign) BLPageControllerCachePolicy cachePolicy;

/** 预加载机制，在停止滑动的时候预加载 n 页 */
@property (nonatomic, assign) BLPageControllerPreloadPolicy preloadPolicy;
...
/** 顶部导航栏 */
@property (nonatomic, weak) BLMenuView * _Nullable menuView;

/** 内部容器 */
@property (nonatomic, weak) BLScrollView * _Nullable scrollView;
 ...
 *  @return instancetype
 */
- (instancetype _Nullable)initWithViewControllerClasses:(NSArray<Class> * _Nullable)classes andTheirTitles:(NSArray<NSString *> * _Nullable)titles;
 ...
 *  @param index 目标序号
 */
- (void)updateTitle:(NSString * _Nullable)title atIndex:(NSInteger)index;
